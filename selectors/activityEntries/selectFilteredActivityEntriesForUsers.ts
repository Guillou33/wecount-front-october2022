...
import { FilterNames, PresenceHashMap } from "@reducers/filters/filtersReducer";
import useAllSiteList from "@hooks/core/useAllSiteList";
import { SiteList } from "@reducers/core/siteReducer";

const selectFilteredActivityEntriesForUsers = createSelector(
  [
    ...
    (state: RootState) =>
      selectSearchableFilter(state, FilterNames.CARTOGRAPHY_PRODUCTS).elementIds,
    (state: RootState) =>
    ...
    (state: RootState) =>
    selectSearchableFilter(state, FilterNames.CARTOGRAPHY_ENTRY_TAGS).elementIds,
  ],
  (
    entries,
    selectedNotExcluded,
...
export default selectFilteredActivityEntriesForUsers;
