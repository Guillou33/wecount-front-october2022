...
const HistoryActivityChart = ({
  historyOfEntries,
  yearSpan,
  activityModelName = upperFirst(t("activity.activities")),
  campaignTypes,
}: Props) => {
  ...
  const historyOfEntriesEmissionColor = arrayProjection(
    [...sortedHistoryOfEntries].reverse(),
    getPalette(Color.EMISSION_BLUE)
  );
  const historyOfEntriesProjectionColor = arrayProjection(
    [...sortedHistoryOfEntries].reverse(),
    getPalette(Color.PROJECTION_PURPLE)
  );

  const series: BarSeriesOption[] = sortedHistoryOfEntries.map(
    (historyId, index): BarSeriesOption => {
      ...
      return {
        ...
        data: years.map(year => {
          ...
          return {
            ...
            itemStyle: {
              color: isProjection
                ? historyOfEntriesProjectionColor[historyId]
                : historyOfEntriesEmissionColor[historyId],
              decal: {
                symbol: isProjection ? "rect" : "none",
                dashArrayX: [1, 0],
                dashArrayY: [3, 4],
                rotation: -Math.PI / 4,
...
export default HistoryActivityChart;
