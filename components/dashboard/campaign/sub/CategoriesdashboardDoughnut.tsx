...
const CategoriesDasboard = ({
  ...
  const options: EChartsOption = {
    ...
    title: {
      text: 'Total: ' + convertToTons(tCo2Total) + ' tCO2e',
      left: 'center',
    },
            ...
            label: {
              formatter: ({ name }) => {
                return `${name}: ${formatPercentageDisplay(
                  item.tCo2,
                  tCo2Total
                ).replace(",",".")}%`;// on garde le formattage anglais exceptionellement pour que echart ne retourne pas à la ligne au niveau de la virgule
              },
            },
            tooltip: {
...
export default CategoriesDasboard;
